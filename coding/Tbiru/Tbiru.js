...
document.addEventListener("DOMContentLoaded", () => {
  const start = async () => {
    try {
      ...
      /* =====================
          VARIABLES UNTUK ZOOM & INTERACTION
      ====================== */
      let initialPinchDistance = null;
      const MIN_SCALE = 0.05; 
      const MAX_SCALE = 0.8; 
      let isPinching = false; // Flag baru untuk zoom

      /* =====================
          BUTTON BACK + INFO + SOUND
      ====================== */
      const backBtn = document.createElement("a");
      backBtn.innerHTML = `<img src="/FYPECO/image-menu/back.png" style="width:100%; height:auto; object-fit:contain;">`;
      backBtn.onclick = (e) => { e.stopPropagation(); window.location.href = "/FYPECO/tongkitarsemula.html"; };
      ...
      Object.assign(instructionOverlay.style, { position: "fixed", inset: "0", background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: "99999" });
      const instructionBox = document.createElement("div");
      Object.assign(instructionBox.style, { background: "#ffffff", padding: "30px", borderRadius: "25px", textAlign: "center", width: "85%" });
      instructionBox.innerHTML = `
        <h2 style="margin-top:0">📱 Cara Interaksi</h2>
        <p>👉 <b>1 Tap</b> : Tukar Paparan<br>👉 <b>Drag</b> : Pusing Model<br>👉 <b>Pinch</b> : Zoom In/Out</p>
        <button id="startARBtn" style="padding:12px 22px; background:#8cd878; border:none; border-radius:18px; font-weight:bold; cursor:pointer;">FAHAM & MULA</button>
      `;
      ...
      Object.assign(progressText.style, { position: "fixed", top: "14px", left: "50%", transform: "translateX(-50%)", fontSize: "20px", fontWeight: "bold", background: "white", padding: "6px 16px", borderRadius: "12px", zIndex: "9999" });
      ...
      Object.assign(progressBarFill.style, { width: "0%", height: "100%", background: "linear-gradient(90deg, #ff9a9e, #fad0c4)", transition: "width 0.3s ease" });
      ...
      // Touch Events (Zoom + Rotate + Tap)
      document.addEventListener("touchstart", e => {
        if (!targetFound) return;
        if (e.touches.length === 1) {
          dragging = true; moved = false; isPinching = false;
      ...
      document.addEventListener("touchend", e => {
        if (!moved && !isPinching && e.changedTouches.length > 0) {
          tryTap(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
        }
        dragging = false; isPinching = false; initialPinchDistance = null;
      });
      ...
      document.addEventListener("mouseup", e => { if (!moved) tryTap(e.clientX, e.clientY); dragging = false; });
